...
import { organization, userTrack, useSession } from "@/lib/auth-client";
      ...
      <div className="flex flex-col gap-6">
        <header>
          ...
          <p className="mt-1 text-sm" style={{ color: "var(--hl-mist)" }}>
            {isRecruiter
              ? "Organization preferences. Jobs, candidates and scores all belong to the organization you set in the header."
              : "Account preferences. Your side, your sessions — nothing here touches recruiter data."}
          </p>
        </header>
        ...
        <TrackCard />
        {isRecruiter && <MembersCard />}
        {isRecruiter && <ApiTokensCard />}
      </div>
    </AppShell>
  );
}
